# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  adifox = {
    pname = "adifox";
    version = "c063db1fe2c6b85365abc81d0367c14b63767a0c";
    src = fetchgit {
      url = "https://github.com/NotAShelf/adifox";
      rev = "c063db1fe2c6b85365abc81d0367c14b63767a0c";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-1YPdderju+1fFf5P3CnD/SSP6P8kSLE0yykEPUJMQvQ=";
    };
    date = "2026-02-07";
  };
  adios = {
    pname = "adios";
    version = "6a54071689ec9ec784a16e4eec8ddb864eca2982";
    src = fetchgit {
      url = "https://github.com/adisbladis/adios";
      rev = "6a54071689ec9ec784a16e4eec8ddb864eca2982";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-NshyMZYZwjKMLKEX0eDf9Fcekq8MFNFk02wW5knzlZU=";
    };
    date = "2026-02-27";
  };
  cyrillic = {
    pname = "cyrillic";
    version = "86186af29eed2af1a069f9e36140d116a2765c80";
    src = fetchFromGitHub {
      owner = "nativerv";
      repo = "cyrillic.nvim";
      rev = "86186af29eed2af1a069f9e36140d116a2765c80";
      fetchSubmodules = false;
      sha256 = "sha256-B2NjvaKJbkih8HLgFAYVqmTuSKAj7XrCBPVoVpYCXXE=";
    };
    date = "2023-07-07";
  };
  heirline-components = {
    pname = "heirline-components";
    version = "5ea9a16286c01b7c36d58c91903d1f8ff0b7ddeb";
    src = fetchFromGitHub {
      owner = "Zeioth";
      repo = "heirline-components.nvim";
      rev = "5ea9a16286c01b7c36d58c91903d1f8ff0b7ddeb";
      fetchSubmodules = false;
      sha256 = "sha256-M86mP8Xr7tIFi9mM8icHWIzbWTR3W2xdSgzXhxNLMj4=";
    };
    date = "2026-02-25";
  };
  helium = {
    pname = "helium";
    version = "0.10.7.1";
    src = fetchurl {
      url = "https://github.com/imputnet/helium-linux/releases/download/0.10.7.1/helium-0.10.7.1-x86_64_linux.tar.xz";
      sha256 = "sha256-ZHziopdl8ClZQJUHXtIIb9ok/flZoixMdlLMKf5HUUo=";
    };
  };
  impermanence = {
    pname = "impermanence";
    version = "7b1d382faf603b6d264f58627330f9faa5cba149";
    src = fetchgit {
      url = "https://github.com/nix-community/impermanence";
      rev = "7b1d382faf603b6d264f58627330f9faa5cba149";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-03+JxvzmfwRu+5JafM0DLbxgHttOQZkUtDWBmeUkN8Y=";
    };
    date = "2026-01-27";
  };
  neorg-conceal-wrap = {
    pname = "neorg-conceal-wrap";
    version = "447a76e4abf07ccc5eeb8cc3ee5fae241b434f5a";
    src = fetchFromGitHub {
      owner = "benlubas";
      repo = "neorg-conceal-wrap";
      rev = "447a76e4abf07ccc5eeb8cc3ee5fae241b434f5a";
      fetchSubmodules = false;
      sha256 = "sha256-nEYuC5bM+VLSvBscpOC3EWwyVWzTPWDvv1o4tAzHzVU=";
    };
    date = "2025-10-17";
  };
  neorg-interim-ls = {
    pname = "neorg-interim-ls";
    version = "e2f86982ba5857ed5581ff5814edd57b5c2c67e0";
    src = fetchFromGitHub {
      owner = "benlubas";
      repo = "neorg-interim-ls";
      rev = "e2f86982ba5857ed5581ff5814edd57b5c2c67e0";
      fetchSubmodules = false;
      sha256 = "sha256-EdKEaxdme4tODtoJv8Yw/x3YtQoDLK0pmTT7xgE1LYQ=";
    };
    date = "2025-10-16";
  };
  neorg-query = {
    pname = "neorg-query";
    version = "64955f82bf6107d8ee05ce9c44bb40edd024b601";
    src = fetchFromGitHub {
      owner = "benlubas";
      repo = "neorg-query";
      rev = "64955f82bf6107d8ee05ce9c44bb40edd024b601";
      fetchSubmodules = false;
      sha256 = "sha256-5qmbZbM0A5mZ5OxxeruNuS9o8FfJymIEHPnHwwxQ5FQ=";
    };
    date = "2026-03-23";
  };
  nix-minecraft = {
    pname = "nix-minecraft";
    version = "253331438df9aaa637c4b13fbac7cce5f6d04775";
    src = fetchgit {
      url = "https://github.com/Infinidoge/nix-minecraft";
      rev = "253331438df9aaa637c4b13fbac7cce5f6d04775";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-oqRN8daUQrUPIjdoc8+bXgy+MVLXt3pa02UeFE/0Eus=";
    };
    date = "2026-04-01";
  };
  nixos-hardware = {
    pname = "nixos-hardware";
    version = "f4c4c2c0c923d7811ac2a63ccc154767e4195337";
    src = fetchgit {
      url = "https://github.com/nixos/nixos-hardware";
      rev = "f4c4c2c0c923d7811ac2a63ccc154767e4195337";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-OrnCQeUO2bqaWUl0lkDWyGWjKsOhtCyd7JSfTedQNUE=";
    };
    date = "2026-03-31";
  };
  nixos-wsl = {
    pname = "nixos-wsl";
    version = "d97e078f4788cddb8d11c3c99f72a4bb9ddec221";
    src = fetchgit {
      url = "https://github.com/nix-community/NixOS-WSL";
      rev = "d97e078f4788cddb8d11c3c99f72a4bb9ddec221";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-DnLIpFxznohpLkIFs390uZ0gxwkVyhtknhKNu+lQJK8=";
    };
    date = "2026-03-31";
  };
  noctalia-dev = {
    pname = "noctalia-dev";
    version = "7b29c11d7aaf476723af14865665cf04750f8d20";
    src = fetchgit {
      url = "https://github.com/noctalia-dev/noctalia-shell";
      rev = "7b29c11d7aaf476723af14865665cf04750f8d20";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-5VzBe0SOXamABAbtjNF5Y7jaMbSeVtGQRhbICoJ6izo=";
    };
    date = "2026-04-01";
  };
  noctalia-qs = {
    pname = "noctalia-qs";
    version = "b87f41771c050d3c101d1528532de53293cba6da";
    src = fetchgit {
      url = "https://github.com/noctalia-dev/noctalia-qs";
      rev = "b87f41771c050d3c101d1528532de53293cba6da";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-zsd/kbsgfX2YQAlbao40JxyeVDwsUmBGyUDZc/Vo1C4=";
    };
    date = "2026-03-31";
  };
  sops-nix = {
    pname = "sops-nix";
    version = "19bf3d8678fbbfbc173beaa0b5b37d37938db301";
    src = fetchgit {
      url = "https://github.com/Mic92/sops-nix";
      rev = "19bf3d8678fbbfbc173beaa0b5b37d37938db301";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-B+rZDPyktGEjOMt8PcHKYmgmKoF+GaNAFJhguktXAo0=";
    };
    date = "2026-03-30";
  };
}
