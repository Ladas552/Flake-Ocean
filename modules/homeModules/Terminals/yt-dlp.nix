  ...
  flake.modules.homeManager.yt-dlp =
    { config, ... }:
    {
      ...
      home.shellAliases = {
        ...
        dl-jpop = "yt-dlp --add-metadata --parse-metadata 'playlist_title:%(album)s' --embed-thumbnail --ignore-errors --format bestaudio --extract-audio --audio-format mp3 --audio-quality 0 --output '${config.xdg.userDirs.music}/jpop/%(playlist_uploader)s/%(playlist)s/%(title)s.%(ext)s' --download-archive '${config.xdg.userDirs.music}/jpop/archive-file' --yes-playlist --max-filesize '20.0M'";
