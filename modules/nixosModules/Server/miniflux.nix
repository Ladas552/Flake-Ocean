{
  flake.modules.nixos.miniflux = args: {
    # secrets
    sops.secrets."mystuff/minifluxl" = { };
    sops.secrets."mystuff/minifluxp" = { };
    sops.templates."miniflux-admin-credentials".content = ''
      ADMIN_USERNAME="${args.config.sops.placeholder."mystuff/minifluxl"}"
      ADMIN_PASSWORD="${args.config.sops.placeholder."mystuff/minifluxp"}"
    '';

    # module
    services.miniflux = {
      enable = true;
      adminCredentialsFile = "${args.config.sops.templates."miniflux-admin-credentials".path}";
      config = {
